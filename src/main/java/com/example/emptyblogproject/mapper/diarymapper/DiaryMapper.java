...
public interface DiaryMapper extends BaseMapper<Diary> {
    ...
    @Select("select   diary.* \n" +
            ...
            "                    group by obj_id , del\n" +
            "                    order by count DESC) as pc  on  pc.obj_id  = diary.id where diary.del = 0 and diary.state = '审批通过' and diary.see = 1 \n" +
    ...
    @Select("select   diary.* from diary left join (select production_star.obj_id , count(*) as count , del\n" +
            ...
            "                    group by obj_id , del\n" +
            "                    order by count DESC) as ps on  ps.obj_id  = diary.id where diary.del = 0 and diary.state = '审批通过' and diary.see = 1 \n" +
